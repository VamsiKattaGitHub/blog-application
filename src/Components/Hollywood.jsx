...
import { NavLink } from "react-router-dom";
 import yourImage from "../images/not-available.jpg"
const Hollywood = () => {
  const [description, setDescription] = useState([]);
  const [loadMoreCards, setLoadMoreCards] = useState(8);
  ...
  const hollyApi = async () => {
    ...
    setDescription(data);
    setLoading(false);
    setTopPosts(data.slice(0, 5)); // Assuming top posts are the first 5 posts
  };
  ...
  const displayedCards = description.slice(0, loadMoreCards);
  const increment = () => {
  ...
  return (
    <>
      <div className="container pb-5">
        <h2 className='mb-4'>Hollywood News</h2>
        <div className="row">
          ...
          <div className="col-md-4">
            ...
            {topPosts.map((post, index) => post.urlToImage == null ?"" : (
              <div className="card mb-3" key={index}>
                <img src={!post.urlToImage? yourImage:post.urlToImage} className="card-img-top" alt={post.title} height={150} style={{objectFit:'cover'}} />
                <div className="card-body">
