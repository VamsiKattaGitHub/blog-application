...
const Fitness = () => {
  ...
  const fitnessApi = async () => {
    const response = await fetch("https://bl0gbackend.onrender.com/fitness");
    const data = await response.json();
    setDescription(data);
    setLoading(false);
    setTopPosts(data.slice(1, 5)); // Assuming top posts are the first 5 posts
  };
  ...
  const displayedCards = description.slice(1, loadMoreCards);
  const increment = () => {
